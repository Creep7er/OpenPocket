package org.openpocket.filepicker
...
class OpenPocketFilePicker(godot: Godot) : GodotPlugin(godot) {
    companion object {
    ...
    override fun getPluginName() = "OpenPocketFilePicker"
            ...
            try {
                val intent = Intent(Intent.ACTION_OPEN_DOCUMENT).apply {
                    ...
                    putExtra(Intent.EXTRA_MIME_TYPES, arrayOf(
                        ...
                        "application/x-openpocket-cartridge"
                    ))
                }
    ...
    override fun onMainActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode != REQUEST_OPEN_CARTRIDGE) return
        if (resultCode != Activity.RESULT_OK || data?.data == null) {
            emitSignal(SELECTION_CANCELLED)
            return
        }
        val uri = data.data ?: return
        executor.execute { importUri(uri) }
    }
    ...
    private fun importUri(uri: Uri) {
        ...
        val output = File(downloads, "import-${UUID.randomUUID()}.pctrg")
        try {
