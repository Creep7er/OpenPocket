...
const PACKAGE_ID := "org.popugonet.popugvpocket.pixelclock"
...
func _draw() -> void:
	...
	PixelFont.draw_text(self, Vector2(16, 16), "PIXEL CLOCK", p["hi"], 2)
	...
	if mode == 3:
		status = "24 HOUR " + ("ON" if bool(PocketStorage.get_package_setting(PACKAGE_ID, "hour_24", true)) else "OFF")
	PixelFont.draw_text(self, Vector2(16, 220), status, p["light"], 1)
	PixelFont.draw_text(self, Vector2(16, 286), "A START/SET  X 12/24  B BACK", p["light"], 1)
