#!/usr/bin/env python3
"""Build, validate, and inspect OpenPocket .pctrg cartridge archives."""
...
def package_manifest_to_cartridge(package_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "version": str(manifest.get("version", "0.3.0-dev")),
        "type": str(manifest.get("type", "game")),
        "entry_scene": f"res://cartridges/{package_id}/{Path(rel_scene).name}",
        "sdk_version": "0.3.2",
        "runtime": {"min_version": "0.3.2", "max_version": None},
        "author": normalize_author(manifest.get("author", "OpenPocket Contributors")),
        "description": str(manifest.get("description", "OpenPocket cartridge.")),
...
def load_cartridge_manifest(package_dir: Path) -> dict[str, Any]:
    ...
    if cartridge_path.exists():
        ...
        manifest["sdk_version"] = "0.3.2"
        manifest["runtime"] = {"min_version": "0.3.2", "max_version": None}
        return manifest
...
def build_content_pack(package_dir: Path, output: Path) -> str:
    ...
    build_root = ROOT / "build" / "cartridge_builder" / f"{cartridge_id}-{manifest['version']}"
    ...
    (build_root / "project.godot").write_text(
        "\n".join([
            "; Generated by OpenPocket Cartridge Builder",
            "config_version=5",
            "",
            "[application]",
...
def command_build(args: argparse.Namespace) -> int:
    print("OpenPocket Cartridge Builder\n")
    output = build(Path(args.package_dir), Path(args.output_dir))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="OpenPocket Cartridge Builder")
